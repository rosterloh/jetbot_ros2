...
std::vector<uint8_t> depacketize(std::vector<uint8_t> packet)
{
  if (packet[0] != UART_START_PACKET) {
    throw jetbotError("Bad packet: did not start with a start byte");
  }
  ...
  if (expected_checksum != received_checksum) {
    throw jetbotError("Bad packet: incorrect checksum");
  }

  return payload;
}

JetBotSerial::JetBotSerial()
: Node("jetbot_serial", rclcpp::NodeOptions().use_intra_process_comms(true))
{
  RCLCPP_INFO(get_logger(), "Starting jetbot serial communication node");
  ...
  pub_raw_data = create_publisher<jetbot_msgs::msg::RawData>("raw_data", rclcpp::QoS(16));
...
void JetBotSerial::on_raw_command(jetbot_msgs::msg::RawCommand::SharedPtr cmd)
{
...
void JetBotSerial::keepalive_callback()
{
...
void JetBotSerial::read_callback()
{
...
void JetBotSerial::on_kill_motors()
{
  motor_efforts_u8 = std::make_shared<std::array<uint8_t, 3>>(MOTOR_EFFORT_HALT);
}

void JetBotSerial::on_motor_efforts(jetbot_msgs::msg::RawMotorCommand::SharedPtr msg)
{
...
unsigned int JetBotSerial::serial_read(std::vector<uint8_t> &inbuf, size_t size)
{
...
unsigned int JetBotSerial::serial_write(const uint8_t *data, size_t length)
{
  unsigned int written_bytes = ::write(*serial_fd_, data, length);
  if ( written_bytes  < length)
  {
    RCLCPP_FATAL(this->get_logger(), "Failed to send command serial command to jetbot");
    throw;
  }
...
unsigned int JetBotSerial::serial_buffer_availible(){
  int bytes_in_buffer;
