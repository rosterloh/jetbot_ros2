...
JetBot::JetBot() : Node("jetbot", rclcpp::NodeOptions().use_intra_process_comms(true))
{
  RCLCPP_INFO(get_logger(), "Starting jetbot driver node");
  ...
  pub_jetbot_command =
    create_publisher<jetbot_msgs::msg::RawCommand>("jetbot_command", rclcpp::QoS(32));
  pub_motor_efforts =
    create_publisher<jetbot_msgs::msg::RawMotorCommand>("motor_efforts", rclcpp::QoS(1));
  ...
  // based on the physical capabilities of the jetbot. Depends on the wheel
  ...
  /// is how far the jetbot has traveled.
  ...
  /// motor encoder, this times n is how far the jetbot has rotated this value
...
void JetBot::on_cmd_vel(geometry_msgs::msg::Twist::ConstSharedPtr msg)
{
...
void JetBot::update_odom()
{
  for (auto arg : {data::LeftMotorEncoderState::Which, data::RightMotorEncoderState::Which,
    ...
    jetbot_msgs::msg::RawCommand cmd;
    cmd.verb = 10;
    cmd.arg = arg;
    pub_jetbot_command->publish(cmd);
  }
  ...
  if (perform_control_loop)
  {
    ...
    jetbot_msgs::msg::RawMotorCommand e;
  ...
  // determine the jetbot's current velocity and publish for state estimation
...
void JetBot::on_raw_data(jetbot_msgs::msg::RawData::ConstSharedPtr data)
{
  most_recent_data[data->which] =
    std::make_unique<Timestamped<data::RawValue>>(get_clock()->now(), data->value);
}

void JetBot::update_power_diagnostics(diagnostic_updater::DiagnosticStatusWrapper & status)
{
...
void JetBot::update_firmware_diagnostics(diagnostic_updater::DiagnosticStatusWrapper & status)
{
  jetbot_msgs::msg::RawCommand cmd;
  cmd.verb = 10;
  cmd.arg = data::JetBotVersion::which();
  pub_jetbot_command->publish(cmd);

  status.clearSummary();

  if (auto data = get_recent<data::JetBotVersion>()) {
...
void JetBot::update_drive_diagnostics(diagnostic_updater::DiagnosticStatusWrapper & status)
{
  // Ask the jetbot to send the next batch of diagnostics data
...
void JetBot::on_reset_pi_controllers()
{
  perform_control_loop = false;
