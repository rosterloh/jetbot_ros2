...
JetBot::JetBot(const std::string & usb_port)
: Node("jetbot_node",rclcpp::NodeOptions().use_intra_process_comms(true))
{
...
JetBot::Wheels* JetBot::get_wheels()
{
  return &wheels_;
}

JetBot::Motors* JetBot::get_motors()
{
  return &motors_;
}

void JetBot::init_dynamixel_sdk_wrapper(const std::string & usb_port)
{
...
void JetBot::check_device_status()
{
...
void JetBot::publish_timer(const std::chrono::milliseconds timeout)
{
...
void JetBot::heartbeat_timer(const std::chrono::milliseconds timeout)
{
...
void JetBot::parameter_event_callback()
{
